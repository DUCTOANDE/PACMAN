...
class GameState:
    ...
    def _render_game(self):
        if self.game_over:
            ...
            if not self.victory_sound_played:
                ...
                # Save ghost movement logs to file
    ...
    def save_ghost_logs(self):
        ...
        try:
            ...
            return # Optionally stop if directory creation fails

        # Create the filename and join it with the directory path
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        filename = f"ghost_movement_log_{timestamp}.json"
        filepath = os.path.join(log_dir, filename) # Use os.path.join for cross-platform compatibility

        # Save the file to the specified path
        try:
            with open(filepath, 'w') as f:
                json.dump(logs, f, indent=4)
            print(f"Ghost movement log saved to: {filepath}") # Optional: confirmation message
        except Exception as e:
            print(f"Error saving ghost movement log to {filepath}: {e}")
